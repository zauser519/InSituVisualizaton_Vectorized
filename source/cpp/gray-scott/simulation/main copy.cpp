...
int main(int argc, char **argv)
{
    ...
    for (int it = restart_step; it < settings.steps;)
    {
        ...
        if(it % settings.plotgap == 0){
            if (rank == 0) {
                printf("Compute Elapsed times for each process:\n");
            }
            printf("It is in step %d, by process %d\n", it, rank);
            printf("Process %d: %f ms\n", rank, timer_compute.elapsed());

            MPI_Barrier(comm);
            if (rank == 0) {
                printf("Write Elapsed times for each process:\n");
            }
            printf("It is in step %d, by process %d\n", it, rank);
            printf("Process %d: %f ms\n", rank, timer_write.elapsed());
        }
    }
